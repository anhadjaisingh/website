...
description: "How to move a subdirectory from one git repository to another while preserving full commit history, even across renames."
date: 2016-07-28
tags: ["git", "linux"]
---

As someone who works in Developer Operations, I'm somtimes presented with interesting requests. A
couple of months back I was asked to _"help move a directory from one git repository to another"_. It
...
**_Except_** when a new request comes in a couple of months later to do another transplant. This time
the request is to transplant something from repository `B` into a third repository `C`.

Easy peasy, been there, done that. _Cough_ right _cough_.
...
... _yep_, it's an empty repository with an init commit, it doesn't change much from the transplant perspective though.
...
_"It doesn't have the history from before the transplant".
"What transplant?".
